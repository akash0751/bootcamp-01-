              ...
              {users && users.map((user,index)=>{
                return(
                  <tr key={user._id}>
                    ...
                    <th>
                      <a onClick={openModel}>
                        <button onClick={()=>getStudentById(user._id)}>View Details</button>
                      </a>
                    </th>
                  </tr>
                )
          ...
          {isModelOpen && (
              <div className='model'>
                  <div className='model-content'>
                      <span onClick={closeModel} className="close">&times;</span><br />
                      <h2 className='h1'>User Record</h2>
                      {
                        idStudents && (
                          <div>
                              <img className="img" src={Image} alt="Student" />
                              <p className='h'>RollNo:{idStudents.rollNo}</p>
                              <p className='h'>Name : {idStudents.name}</p>
                              {isVisible && (
                                <a onClick={()=>SetISVisible(false)}>
                                  <button onClick={openDiv} className='btn1'>View More</button>
                                </a>  
                              )}
                          </div>
                        ) 
                        ...
                        isDivOpen && (
                          <>
                            ...
                            <a onClick={()=>SetISVisible(true)}>
                              <button className="btn1" onClick={closeDiv}>View Less</button>
                            </a>
                          </>
                        )
                      }
